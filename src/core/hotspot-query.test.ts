...
function makeHotspot(overrides: Partial<HotspotView> = {}): HotspotView {
  return {
    ...
    keywordMentioned: overrides.keywordMentioned ?? true,
    matchType: overrides.matchType ?? "exact",
    matchedTerms: overrides.matchedTerms ?? ["OpenAI"],
    missingRequiredTerms: overrides.missingRequiredTerms ?? [],
    queryExpansionTerms: overrides.queryExpansionTerms ?? ["OpenAI"],
    whyRelevant: overrides.whyRelevant ?? "Direct keyword mention",
    whyNotRelevant: overrides.whyNotRelevant ?? "No major mismatch",
